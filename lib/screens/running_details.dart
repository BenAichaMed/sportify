...
class RunningChallengeDetailScreen extends StatefulWidget {
  final RunningCyclingChallenge challenge;

  const RunningChallengeDetailScreen({super.key, required this.challenge});

  @override
  _RunningChallengeDetailScreenState createState() =>
      _RunningChallengeDetailScreenState();
}

class _RunningChallengeDetailScreenState
    extends State<RunningChallengeDetailScreen> {
  bool isJoined = false;
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
              'You have successfully joined the challenge!. you can now chat with other participants just click the chat icon on the top right corner'),
          actions: <Widget>[
            TextButton(
              child: const Text('OK'),
              onPressed: () {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (!participantsList.contains(userId)) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('You are not a participant.')),
          );
          return;
        }
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 30),
                      Text(
                        widget.challenge.distance.toString()+" km",
                        style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 18),
                      ),
                    ],
                  ),
